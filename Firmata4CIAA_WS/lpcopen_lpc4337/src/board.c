...
{
	if (pUART == LPC_USART0) {
		Chip_SCU_PinMuxSet(0x6, 4, (SCU_MODE_PULLDOWN | SCU_MODE_FUNC2));					/* P6.4 : UART0_TXD */
...
void Board_LED_Set(uint8_t LEDNumber, uint32_t value)
{
	Chip_GPIO_SetPinState( LPC_GPIO_PORT, outputs[LEDNumber].port, outputs[LEDNumber].bit, value);
}

uint32_t Board_LED_Test(uint8_t LEDNumber)
{
	return Chip_GPIO_GetPinState( LPC_GPIO_PORT, outputs[LEDNumber].port, outputs[LEDNumber].bit );
}
...
uint32_t Buttons_GetStatus(uint32_t inputNumber)
{
	return (Chip_GPIO_ReadPortBit(LPC_GPIO_PORT,
			inputs[inputNumber].port,
			inputs[inputNumber].bit) == 0)?1:0;
}
